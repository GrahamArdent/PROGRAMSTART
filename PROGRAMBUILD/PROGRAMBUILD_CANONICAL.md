...
18. PROGRAMSTART acceptance learning MUST remain evidence-driven and subordinate to product execution. The dominant Work Packet closure path MUST evaluate the existing Learning Gate at meaningful checkpoints before close/handoff, while `no reusable lesson` remains a valid result requiring no durable learning write. A resolved material Authority Gap that exposed cross-project mismatch, systemic PROGRAMSTART friction/failure, or counterevidence to a prior methodology assumption is itself a meaningful Learning Gate checkpoint. Product completion MUST NOT depend on the ability to mutate the PROGRAMSTART repository, and the learning ledger MUST NOT become a product backlog, portfolio Master, or execution authority.
...
25. Progressive / just-in-time context loading MUST remain anchored to the selected objective's authority-derived terminal condition. Material discoveries MUST be classified against that terminal condition: blocking or required dependencies are resolved/routed before truthful closure; useful non-blocking findings MAY be preserved/routed without abandoning the objective; unrelated findings MUST NOT silently widen the packet. A locally green intermediate artifact MUST NOT be treated as completion while authorized unattended-safe work remains to reach the terminal condition.
...
| planning-to-execution separation, objective-level convergence and authority-derived terminal-condition semantics, idea capture/promotion semantics, proportional rigor, blocker scope/safe-lane reasoning, adaptive decision/evidence routing, accepted-recommendation resolution, checklist activation rules, context loading, and evidence reuse | `PROGRAMBUILD_PLANNING_OPERATING_MODEL.md` |
