...
- [ ] for a credential / identity / trust / access gap, recover existing accepted capability first and run the Effective Autonomy human-enablement leverage test before expensive workaround engineering; when bounded human enablement clearly wins, prepare everything delegable, present only the irreducible secure-surface action, verify it mechanically, and auto-resume; do not generalize this into human-first behavior
