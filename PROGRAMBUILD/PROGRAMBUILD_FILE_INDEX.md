...
| `PROGRAMBUILD_WORK_PACKET.md` | template/protocol | active | compact logical work-packet semantics, accepted-recommendation disposition/gate evidence, checklist completeness/closure, conditional concept-viability and bounded-exploration state, human-gate readiness/human-only remainder, blocker/safe-lane fields, coordinated Mode-C lane selection with exclusive shared-mutation ownership when consequential resources overlap, task-scoped cross-repository dependency/authority evidence, operator/manual-gate handoff semantics, bounded cross-system test envelopes, plus optional persisted format | logical work-packet semantics |
...
| `PROGRAMBUILD_CHECKLIST.md` | checklist | active | reusable execution checklist plus derived checklist activation/source/status/closure discipline, including concept-falsification/retry-delta discipline, Effective Autonomy alternative-actuation checks, and human-gate readiness before operator transport | execution tracking |
...
| `PROGRAMBUILD_CHALLENGE_GATE.md` | protocol | active | A–H risk controls plus variant/stage/risk-based gate-part selection, post-implementation failure-sequence review, concept/retry/human-gate trajectory challenge, and cross-system ownership challenge | stage/convergence gates |
...
| File | Type | Purpose |
|---|---|---|
| `AGENTS.md` | repository execution contract | execution-facing instructions automatically consumable by Codex-compatible `AGENTS.md` workflows; routes agents to current authority, concept-viability/human-gate discipline, verification, and final self-review without becoming project scope/sequence authority |
| `.github/agents/*.agent.md` | specialist workspace-agent definitions | optional bounded specialist roles referenced by `PROGRAMBUILD_SUBAGENTS.md`; not a substitute for root `AGENTS.md` and not project authority |
...
| `docs/PROGRAMSTART_EFFECTIVE_AUTONOMY.md` | operational protocol | resolves effective autonomy as the intersection of project consequence authority, PROGRAMSTART governance, current Controller/Compute/worker/identity capability and evidence freshness; owns conditional concept viability/falsification and information-value-bounded exploration; requires proportional alternative-actuation search/composition before operator transport and technical human-gate readiness with an explicit human-only remainder while preserving stronger gates | consequence-scoped autonomy, concept viability, and manual-boundary resolution |
