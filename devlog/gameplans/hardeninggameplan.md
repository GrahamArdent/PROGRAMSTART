...
#### ✅ J-3: Mutation testing (T-5) — COMPLETE

**Status**: COMPLETE
**Checkpoint**: Added `mutmut` to the dev dependency set, defined a focused `[tool.mutmut]` configuration for `scripts/programstart_recommend.py`, and hardened the canonical `nox -s mutation` path so it now wipes the generated `mutants/` workspace before each run, ignores that workspace in git, rebuilds a fresh WSL mutation environment on Windows, and fails closed if `mutmut` exits without producing materialized mutant outcomes in `mutants/scripts/programstart_recommend.py.meta`. The first trustworthy canonical run completed on 2026-04-16 via `uv run nox -s mutation` with `total=3091`, `pending=0`, `killed=1267`, `survived=1824`, `other=0`, at roughly `3.62 mutations/second`. A second completed canonical run, after the first mutation-driven test pass, finished the same day with `total=3247`, `pending=0`, `killed=1612`, `survived=1635`, `other=0`, at roughly `4.80 mutations/second`. A third completed canonical run, measuring the follow-up tests added for `infer_domain_names()`, `print_recommendation()`, and `main()`, finished the same day with `total=3263`, `pending=0`, `killed=1671`, `survived=1592`, `other=0`, at roughly `5.04 mutations/second`. A fourth completed canonical run, measuring the tightened `shape_profile()` assertions plus the stronger `build_stack_candidates()` exact-output coverage, finished the same day with `total=3263`, `pending=0`, `killed=1866`, `survived=1397`, `other=0`, at roughly `3.71 mutations/second`. A fifth completed canonical run, measuring the stricter `infer_domain_names()` ordering case, the exact scoring-heavy `build_stack_candidates()` path, and the lean `build_recommendation()` path, finished the same day with `total=3263`, `pending=0`, `killed=1968`, `survived=1295`, `other=0`, at roughly `5.20 mutations/second`. A sixth completed canonical run, measuring the exact helper assertions added for `comparison_bonus()`, `preferred_rule_items_for_layer()`, and third-party API selection in `select_triggered_entries()`, finished the same day with `total=3268`, `pending=0`, `killed=2029`, `survived=1239`, `other=0`, at roughly `4.92 mutations/second`. A seventh completed canonical run, measuring the broader alias-ordering assertions for `infer_domain_names()`, the denser regulated dashboard scoring path for `build_stack_candidates()`, and the lite CLI assembly path for `build_recommendation()`, finished the same day with `total=3268`, `pending=0`, `killed=2090`, `survived=1178`, `other=0`, at roughly `4.30 mutations/second`. An eighth completed canonical run, measuring the denser multi-trigger `select_triggered_entries()` path, the realtime workflow scoring path for `build_stack_candidates()`, and the full actionability-plus-advisory `build_recommendation()` path, finished the same day with `total=3281`, `pending=0`, `killed=2114`, `survived=1167`, `other=0`, at roughly `4.33 mutations/second`. A ninth completed canonical run, measuring the broader alias-family `infer_domain_names()` case, the governed API seed-and-partial scoring path for `build_stack_candidates()`, and the web-app `build_recommendation()` path with default USERJOURNEY attachment and cross-layer alternative dedupe, finished the same day with `total=3281`, `pending=0`, `killed=2127`, `survived=1154`, `other=0`, at roughly `3.96 mutations/second`. A tenth completed canonical run, measuring the new provisioning-service `select_triggered_entries()` case, the authenticated dashboard `build_stack_candidates()` path, and the data-pipeline `build_recommendation()` companion-surface advisory path, finished the same day with `total=3281`, `pending=0`, `killed=2148`, `survived=1133`, `other=0`, at roughly `5.40 mutations/second`. An eleventh completed canonical run, measuring the feature-flag API `select_triggered_entries()` case, the feature-flag control-plane `build_stack_candidates()` path, and the regulated-API `build_recommendation()` path, finished the same day with `total=3281`, `pending=0`, `killed=2165`, `survived=1116`, `other=0`, at roughly `4.09 mutations/second`. A twelfth completed canonical run, measuring the new `build_stack_candidates()` exact-output case for unknown coverage status, tradeoff-penalty reasoning, and fallback gap text, finished the same day with `total=3281`, `pending=0`, `killed=2189`, `survived=1092`, `other=0`, at roughly `3.59 mutations/second`. Relative to the eleventh completed run, that added `24` killed mutants and removed `24` survivors with no change in the total mutant count.
A thirteenth completed canonical run on 2026-04-17, after adding 51 assertion-dense integration-level tests targeting `build_stack_candidates()`, `build_recommendation()`, `main()`, and `print_recommendation()` via a controlled `_scoring_kb()` fixture, finished with `total=3253`, `pending=0`, `killed=1599`, `survived=1652`, `other=2`, at roughly `3.62 mutations/second`. This is a regression from the twelfth run (killed 2189 → 1599, survived 1092 → 1652). Root cause: `mutate_only_covered_lines = true` regenerated a different mutation set because the new tests' coverage profile exercised previously-uncovered branches, changing which source lines received mutations. The mutation set is not stable across test-suite changes, so absolute killed/survived counts between runs with different test suites are not directly comparable. Per-function survivor breakdown for run 13: `build_stack_candidates` 318/669 (48%), `select_triggered_entries` 175/297 (59%), `build_recommendation` 155/372 (42%), `infer_domain_names` 145/177 (82%), `shape_profile` 144/194 (74%), `print_recommendation` 82/193 (42%), `main` 75/208 (36%), `comparison_bonus` 68/78 (87%), `build_generated_prompt` 63/77 (82%), `build_actionability_summary` 59/124 (48%), `re_evaluate_project` 59/117 (50%).

A fourteenth completed canonical run on 2026-04-17, after adding ~47 unit-level exact-value tests targeting `comparison_bonus()`, `actionability_follow_up_commands()`, `infer_domain_names()`, `build_generated_prompt()`, and `shape_profile()`, finished with `total=3257`, `pending=0`, `killed=1854`, `survived=1402`, `other=1`, at roughly `3.70 mutations/second`. This is a +7.8pp improvement over run 13 (49.1% → 56.9% kill rate). The mutation set was nearly stable (3257 vs 3253 — only 4 mutant difference). Per-function survivor comparison (run 13 → run 14 surv%): `comparison_bonus` 87% → 17.1%, `actionability_follow_up_commands` 88% → 17.6%, `shape_profile` 74% → 17.5%, `build_generated_prompt` 82% → 39.0%, `infer_domain_names` 82% → 65.5%. The five targeted functions all improved dramatically. Remaining high-survivor functions: `choose_cli_tool_names` 72.1%, `choose_service_names` 69.2%, `choose_api_names` 69.2%, `infer_domain_names` 65.5%, `select_triggered_entries` 58.9%, `infer_variant` 52.6%, `stack_entry_text` 51.1%, `load_recommendation_inputs` 51.6%, `preferred_rule_items_for_layer` 50.7%, `re_evaluate_project` 50.4%.

**Current blocker**: Diminishing returns. The five highest-survivor helper functions were brought under control (87–74% → 17–39% survivor rate). The remaining survivors are concentrated in larger orchestration functions (`build_stack_candidates` 318 survivors, `select_triggered_entries` 175, `build_recommendation` 155, `infer_domain_names` 116) where mutations are predominantly in string formatting, output assembly, and conditional branches that require full-system integration fixtures to exercise — the cost-per-kill is high and the safety value is low.

**Decision**: Close J-3. The kill rate improved from 41.0% (run 1) to 56.9% (run 14) over 14 canonical runs. The unit-level test pass confirmed that targeted exact-value assertions are the most efficient mutation killers. Further improvement requires either broadening the mutation target to other scripts or accepting the remaining survivors as low-risk string-formatting mutations. Neither action justifies keeping J-3 open.

**Next pass**: Move to J-4 (Registry Pydantic models) or other high-value hardening work. If mutation testing is revisited later, start by targeting the `choose_cli_tool_names` / `choose_service_names` / `choose_api_names` family (small functions, high survivor rates, likely similar mutation patterns).
