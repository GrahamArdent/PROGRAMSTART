...
**Current blocker**: The blocker is mutation set instability and survivor density in helper functions. The `mutate_only_covered_lines = true` setting causes the mutation population to shift when coverage changes, defeating run-over-run progress tracking. Additionally, high-survivor-rate functions (`comparison_bonus` 87%, `actionability_follow_up_commands` 88%, `infer_domain_names` 82%, `build_generated_prompt` 82%, `shape_profile` 74%) have only type-checking or integration-level tests — they need unit-level exact-value assertions to kill fine-grained mutations.

**Decision**: Keep J-3 open and do not broaden mutation coverage beyond `scripts/programstart_recommend.py` yet.

**Next pass**: Write unit-level tests with exact value assertions for the highest survivor-rate helper functions: `comparison_bonus()`, `actionability_follow_up_commands()`, `infer_domain_names()`, `build_generated_prompt()`, and `shape_profile()`. These functions have the worst survivor rates (74–88%) and are small enough for exhaustive unit coverage. After adding those, re-run `uv run nox -s mutation` to measure the impact on the same coverage profile.
