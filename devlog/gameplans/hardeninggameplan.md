...
**Current blocker**: The blocker is no longer runner trustworthiness. It is now the survivor volume and survivor explanation. The completed runs proved the hardened path materializes real outcomes and rejects the earlier bad states (truncated JSON metadata, false `0/0`, and all-pending metadata after a long rebuild), but J-3 remains open because survivors are still concentrated in recommendation scoring and presentation logic rather than a small explainable tail.

**Decision**: Keep J-3 open and do not broaden mutation coverage beyond `scripts/programstart_recommend.py` yet.

**Next pass**: Keep `uv run nox -s mutation` as the canonical entry point, but continue survivor triage with the updated hotspot ordering from the twelfth completed run: `build_stack_candidates()` (around line 601), `build_recommendation()` (around line 1033), `main()` (around line 1375), `infer_domain_names()` (around line 331), `select_triggered_entries()` (around line 483), `re_evaluate_project()` (around line 1251), and `print_recommendation()` (around line 1299). The latest measured run held `build_stack_candidates()` flat at `180` survivors, cut `build_recommendation()` from `102` to `100`, held `select_triggered_entries()` at `85`, and pulled the overall tail down by another `24` survivors while holding the total mutant count flat at `3281`. The immediate next action is another focused survivor-reduction test pass against `build_stack_candidates()`, `build_recommendation()`, and `main()` before broadening mutation coverage.
