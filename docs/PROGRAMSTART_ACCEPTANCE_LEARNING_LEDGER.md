...
| `PSL-018` | Operators managing many independent repositories need a lightweight derived portfolio-attention layer that separates short operator gates from one primary build, keeps live filled state outside PROGRAMSTART, treats staleness as verification debt rather than urgency, and hands execution back to each project's authority | **validated** | `PROGRAMBUILD_PORTFOLIO_CONTROL.md` + external-workspace templates | external `GrahamArdent/portfolio-operations` live refresh/reselection retest; `docs/acceptance/observations/2026-08-31-portfolio-attention-live-retest.md` | continue normal use; record counterevidence if refresh overhead, stale misrouting, shadow-backlog drift, or authority leakage appears |
