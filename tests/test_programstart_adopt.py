...
def _registry() -> dict:
    return {
        ...
        "prompt_authority": {
            ".github/prompts/stage.prompt.md": {"authority_files": ["PROGRAMBUILD/PROGRAMBUILD.md"]}
        },
...
def _fake_bootstrap(destination: Path, _registry: dict, variant: str, dry_run: bool) -> None:
    ...
    (pb / "PROGRAMBUILD_STATE.json").write_text(json.dumps({"variant": variant}) + "\n", encoding="utf-8")
    (pb / "REQUIREMENTS.md").write_text("Purpose: Requirements\n---\n\n", encoding="utf-8")


def test_adopt_preserves_host_toolchain_and_tracks_only_managed_methodology(tmp_path: Path, monkeypatch) -> None:
    ...
    monkeypatch.setattr(adopt, "_managed_prompt_assets", lambda _registry: (".github/prompts/stage.prompt.md",))
...
def test_adopt_dry_run_does_not_modify_repo(tmp_path: Path, monkeypatch, capsys) -> None:
    ...
    adopt.adopt_programbuild(destination, project_name="Existing App", variant="lite", dry_run=True)
