...
def test_partial_evidence_custom_stop_fields_are_preserved() -> None:
    result = route_decision(
        DecisionContext(
...
def test_unified_cli_decide_dispatch(monkeypatch) -> None:
    ...
    assert programstart_cli.main(
        ["decide", "--decision", "Choose next step", "--evidence", "sufficient"]
    ) == 0
    assert captured == [
        ["programstart decide", "--decision", "Choose next step", "--evidence", "sufficient"]
    ]
